...
public class ActivitiServiceImpl implements ActivitiService {
    ...
    public String getformkey(String tid) {
        System.out.println("getformket-tid"+tid);
        TaskFormData tfd = formService.getTaskFormData(tid);
    ...
    public void chuli(String taskid, String pizhu, String result, int leaveid,String username) {
        ...
        System.out.println("pizhu+"+pizhu);
        System.out.println("task.getProcessInstanceId()+"+task.getProcessInstanceId());
        System.out.println("taskid+"+taskid);
        Comment comment = taskService.addComment(taskid,task.getProcessInstanceId(),pizhu);
        //2.设置流程变量的值
        Map map=new HashMap();
        map.put("rs",result);
        //完成个人任务
    ...
    public List<Comment> getcomments(String tid) {
        ...
        //得到任务的流程实例id
        Task task = taskService.createTaskQuery().taskId(tid).singleResult();
        String instanceid = task.getProcessDefinitionId();
